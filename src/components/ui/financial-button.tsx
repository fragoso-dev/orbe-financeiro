...
const financialButtonVariants = cva(
  "inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-lg text-sm font-medium transition-smooth focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0",
  {
    variants: {
      variant: {
        ...
        gradient: "gradient-primary text-primary-foreground shadow-lg hover:opacity-90",
      },
      size: {
...
export interface FinancialButtonProps
  extends React.ButtonHTMLAttributes<HTMLButtonElement>,
    VariantProps<typeof financialButtonVariants> {
  asChild?: boolean
}

const FinancialButton = React.forwardRef<HTMLButtonElement, FinancialButtonProps>(
  ({ className, variant, size, asChild = false, ...props }, ref) => {
    const Comp = asChild ? Slot : "button"
    return (
      <Comp
        className={cn(financialButtonVariants({ variant, size, className }))}
        ref={ref}
        {...props}
      />
    )
...
export { FinancialButton, financialButtonVariants }